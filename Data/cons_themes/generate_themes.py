...
# النسبة الذهبية عشان نوزع الألوان بشكل مثالي ومفيش لونين يبقوا متطابقين
GOLDEN_RATIO = 0.618033988749895

for i in range(6, 3001):
    # توزيع درجات الألوان (Hue) باستخدام النسبة الذهبية
    hue = (i * GOLDEN_RATIO) % 1.0
    
    # تغيير ديناميكي في التشبع (Saturation) والإضاءة (Lightness) بناءً على رقم اللوب
    s_bg = 0.15 + (i % 20) / 100.0     # التشبع للخلفية (من 0.15 لـ 0.34)
    l_bg = 0.08 + (i % 15) / 100.0     # الإضاءة للخلفية (من 0.08 لـ 0.22) عشان تفضل غامقة وشيك
    
    s_txt = 0.4 + (i % 30) / 100.0     # التشبع للنص (من 0.4 لـ 0.69)
    l_txt = 0.75 + (i % 15) / 100.0    # إضاءة النص (من 0.75 لـ 0.89) عشان يكون واضح ومقروء
    
    s_cov = min(1.0, s_bg + 0.1)       # غلاف الشيخ أزهى سنة من الخلفية
    l_cov = min(1.0, l_bg + 0.1)       # غلاف الشيخ أفتح درجة من الخلفية
